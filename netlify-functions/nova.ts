...
const ORIGINS = ['https://www.fitfi.ai','https://fitfi.ai','https://fitfi.netlify.app','http://localhost:5173'];

function corsOrigin(o: string | null){ try{
  if (!o) return ORIGINS[0]; if (ORIGINS.includes(o)) return o;
  const u=new URL(o); if ((u.protocol==='https:'||u.protocol==='http:')&&u.hostname.endsWith('.netlify.app')) return o;
...
function parseCookies(h: string| null){ const out:Record<string,string> = {}; if(!h) return out;
  h.split(';').forEach(p=>{ const i=p.indexOf('='); if(i>0){ out[p.slice(0,i).trim()]=decodeURIComponent(p.slice(i+1)); } }); return out; }

function isoDay(d=new Date()){ return d.toISOString().slice(0,10); }
function isoWeek(d=new Date()){ const dt=new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),d.getUTCDate()));
...
function ub64u(s:string){ return Buffer.from(s,'base64url').toString(); }
async function hmac(input:string, secret:string){ const { createHmac } = await import('crypto'); return createHmac('sha256', secret).update(input).digest('hex'); }
...
export default async (req: Request) => {
  ...
  if (req.method==='GET') {
    return new Response(JSON.stringify({
      ok:true, fn:'nova', node:process.version, hasKey: !!process.env.OPENAI_API_KEY
  ...
  if (!key) {
    const rs = new ReadableStream({ start(c){ c.enqueue(`data: ${JSON.stringify({type:'error', message:'missing OPENAI_API_KEY'})}\n\n`); c.enqueue('data: {"type":"done"}\n\n'); c.close(); }});
    return new Response(rs,{ status:200, headers:{ 'Content-Type':'text/event-stream; charset=utf-8','Cache-Control':'no-cache, no-transform','Connection':'keep-alive','Access-Control-Allow-Origin': allow }});
  }

  // Parse body
  const { mode, messages, stream } = await req.json() as { mode: Mode; messages: Msg[]; stream?: boolean; };
  const m: Mode = (['outfits','archetype','shop'] as Mode[]).includes(mode) ? mode : 'outfits';
  if (!Array.isArray(messages) || !messages.length) return new Response('Bad request',{ status:400, headers:{'Access-Control-Allow-Origin': allow }});
  ...
  if (!state) state = { uid: hdrUid || (cookies['fitfi_uid']||''), tier: hdrTier, day: isoDay(), week: isoWeek(), counts: { outfits: 0 } };
  ...
  const today = isoDay(); if (state.day !== today) { state.day = today; state.counts = { outfits: 0 }; }
  const thisWeek = isoWeek(); if (state.week !== thisWeek) { state.week = thisWeek; /* week tellen impliciet via dagcount som; simpel houden */ }
  ...
  if (!withinMode || overDay || overWeek) {
    ...
    const rs = new ReadableStream({
      start(controller) {
        ...
        send({ type:'error', code:'quota_exceeded', tier, message: overDay ? 'Daglimiet bereikt' : 'Niet toegestaan in deze tier' });
    ...
    headers['Set-Cookie'] = `fitfi_uid=${encodeURIComponent(state.uid)}; Path=/; Max-Age=31536000; SameSite=Lax; Secure`;
  ...
  if (secret) {
    ...
    setCookies.push(`fitfi_usage=${b64u(payload)}.${sig}; Path=/; Max-Age=604800; SameSite=Lax; Secure`);
  }
      ...
      start: async (controller) => {
        ...
        try {
          const upstream = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({ model: routeModel(m), messages: [{role:'system',content:systemPrompt(m)}, ...messages], stream: true, temperature: 0.7 }),
            // @ts-ignore
            duplex: 'half'
          });
          ...
          if (!upstream.ok || !upstream.body) {
            ...
            send({ type:'error', message:'upstream error', status: upstream.status, detail: txt.slice(0,200) });
          ...
          send({ type:'error', message:'network error' }); send({ type:'done' }); controller.close();
        }
      }
    });
  ...
  // JSON fallback
  return new Response(JSON.stringify({ content:'Nova (fallback): streaming niet actief.' }), {
    headers: { 'Access-Control-Allow-Origin': allow, 'Content-Type':'application/json', 'Set-Cookie': setCookies.join(', ') }
  });
}
