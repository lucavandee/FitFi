...
try {
  ...
  const version = execSync("npx supabase --version", {
    encoding: "utf8",
  }).trim();
  ...
  if (ACCESS_TOKEN) {
    ...
    execSync("npx supabase auth login --token $SUPABASE_ACCESS_TOKEN", {
      stdio: "inherit",
    });
  ...
  execSync('npx supabase db commit -m "baseline: synced production schema"', {
    stdio: "inherit",
  });
  ...
  console.log("\n📚 Next steps:");
  console.log(
    "   • Create new migrations: npx supabase db diff -f <migration_name>",
  );
