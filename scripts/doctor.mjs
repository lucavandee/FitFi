...
async function walk(dir) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      // skip node_modules/dist/build/temp
...
const HEX_RE = /#[0-9a-fA-F]{3,8}\b/g; // 3–8 hex (incl. #RRGGBBAA)
...
async function checkHexColors() {
  ...
  for (const f of files) {
    ...
    // Tailwind arbitrary values met hex (#fff) zijn óók niet toegestaan buiten tokens
...
async function checkHostLeftovers() {
  ...
  if (offenders.length) {
    ...
    for (const p of offenders.slice(0, 100)) console.warn(`   • ${p}`);
    if (offenders.length > 100) console.warn(`   … +${offenders.length - 100} extra bestanden`);
  } else {
    ...
    try {
      ...
      proc.on("close", (code) => {
        if (code === 0) {
          ...
          console.warn(out.trim().slice(0, 8000)); // truncate
        }
        resolve(code);
      });
    } catch (e) {
...
(async function main() {
  ...
  const host = await checkHostLeftovers();
  await runTscNoEmit();

  const summary = [
    `HEX-kleur overtredingen: ${hex}`,
    `fitfi.app verwijzingen: ${host}`
  ].join(" | ");

  console.log(`\n🧾  Samenvatting: ${summary}`);
  console.log("\nℹ️  Deze checks falen de build NIET. Stel CI_TOKENS_STRICT=1 in om ze in CI te forceren (toekomstig).");
  process.exit(0);
})();