...
const OutfitCard: React.FC<OutfitCardProps> = ({
  ...
  return (
    <article
      ref={ref}
      className={`group rounded-2xl bg-[#FFFFFF] overflow-hidden transition-all duration-300 ${
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-[1px] bg-[#E5E5E5]">
        <div className="relative col-span-1 row-span-1 aspect-[3/4] bg-[#FAFAF8]">
          <SmartImage
        ...
        <div className="grid grid-rows-3 gap-[1px] bg-[#E5E5E5]">
          {[gridImages[1], gridImages[2], gridImages[3]].map((src, i) => (
            <div key={i} className="relative overflow-hidden bg-[#FAFAF8]">
              <SmartImage
      ...
      <div className="p-3">
        ...
        <div className="flex items-start justify-between gap-2 mb-1">
          <h3
            className="text-[13px] font-bold text-[#1A1A1A] leading-snug flex-1"
            style={{
              display: '-webkit-box',
              WebkitLineClamp: 2,
        ...
        {isOverBudget && (
          <span className="inline-flex items-center gap-1 px-1.5 py-px rounded text-[10px] font-semibold bg-[#D4913D] text-white mb-1.5">
            Boven budget
          </span>
        )}
