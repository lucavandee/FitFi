...
export function ResultsOutfitCard({
  ...
  return (
    <article
      onClick={onSelect}
      className="bg-white border border-[#E5E5E5] rounded-2xl overflow-hidden hover:shadow-[0_12px_40px_rgba(0,0,0,0.08)] hover:border-[#B55E45] transition-all duration-300 cursor-pointer group"
    >
      ...
      <div className="relative aspect-[3/4] overflow-hidden bg-[#F5F0EB]">
        {imageUrl ? (
          ...
          <div className="w-full h-full bg-gradient-to-br from-[#F4E8E3] to-[#F5F0EB] flex items-center justify-center">
            <div className="text-center p-4">
              <Sparkles className="w-10 h-10 mx-auto mb-2 text-[#B55E45]" aria-hidden="true" />
        ...
        >
          <Heart className={`w-4 h-4 ${isFavorite ? "fill-[#B55E45]" : ""}`} />
        </button>
      </div>
          ...
          <div className="flex items-center gap-3">
            {typeof matchScore === "number" && (
              <span className="inline-flex items-center gap-1.5">
                ...
                <span className="text-sm font-semibold text-[#B55E45]">{Math.round(matchScore)}% match</span>
              </span>
            )}
            {fitLabel && (
