...
function ScorePill({ score }: { score: number }) {
  ...
  return (
    <div
      className="absolute top-3 right-3 rounded-full px-2 py-1 text-xs font-semibold text-white"
...
function ShopLink({ o }: { o: Outfit }) {
  const q = encodeURIComponent(
    o.shopQuery || o.items.map((i) => i.name).join(" "),
  );
  return (
    <a
      href={`/shop?q=${q}`}
  ...
  return (
    <div className="mt-3 grid grid-cols-1 sm:grid-cols-2 gap-4">
      {data.outfits.map((o) => (
        <div
          key={o.id}
          className="relative rounded-2xl border bg-white p-4 shadow-[0_8px_30px_rgb(13_27_42/0.06)]"
        >
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              {o.occasion && (
                <div className="text-xs text-gray-500 mt-0.5">{o.occasion}</div>
              )}
            </div>
          ...
          <ul className="mt-3 space-y-1.5 text-sm text-ink">
            {o.items.map((it, idx) => (
              <li key={idx} className="flex gap-2">
                ...
                {it.color && (
                  <span className="text-gray-500">({it.color})</span>
                )}
          ...
          {blur && (
            <div className="absolute inset-0 rounded-2xl backdrop-blur-sm bg-white/45 border-dashed border-2 border-[#89CFF0]/50 flex items-center justify-center">
              <div className="text-[#0D1B2A] font-medium">
                Word Member (gratis) om te zien
              </div>
            </div>
          )}
        </div>
