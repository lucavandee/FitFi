    ...
    <div className={`bg-white rounded-2xl p-6 shadow-sm ${className}`}>
      ...
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-lg font-bold text-gray-900 flex items-center">
          <Trophy className="w-5 h-5 text-[#A85740] mr-2" />
        ...
        {entries.map((entry, index) => (
          <div
            ...
            className={`flex items-center space-x-4 p-3 rounded-xl border transition-all hover:shadow-sm ${getRankColor(entry.rank)} ${
              entry.isCurrentUser ? 'ring-2 ring-[#A85740]' : ''
            }`}
          ...
          >
            ...
            <div className="flex-1">
              <div className="font-medium text-gray-900">
                {entry.name}
                {entry.isCurrentUser && (
                  <span className="ml-2 text-xs bg-[#A85740] text-white px-2 py-1 rounded-full">
                    Jij
                  </span>
                )}
              </div>
      ...
      {showCurrentUser && currentUserRank && currentUserRank > limit && (
        <div className="mt-6 pt-4 border-t border-gray-200">
          <div className="flex items-center space-x-4 p-3 rounded-xl bg-[#A85740]/10 border border-[#A85740]/20">
            <div className="w-8 h-8 flex items-center justify-center">
              <span className="text-sm font-bold text-[#A85740]">#{currentUserRank}</span>
            </div>
