...
interface FoundersBlockDashboardProps {
  className?: string;
}

const FoundersBlockDashboard: React.FC<FoundersBlockDashboardProps> = ({ className = '' }) => {
  const { user } = useUser();
  
  // Guard: Return null if no user (prevents empty rendering)
  if (!user) {
    return null;
  }
            ...
            >
              ...
              <div className="flex justify-center lg:justify-start">
                <div className="relative w-32 h-32">
                  <svg className="w-32 h-32 transform -rotate-90 progress-bar" viewBox="0 0 100 100">
                    {/* Background circle */}
                    <circle
                      cx="50"
                      cy="50"
