...
const StylePreview: React.FC<StylePreviewProps> = ({ answers, currentStep, className = '' }) => {
  ...
  return (
    <div className={`bg-gradient-to-br from-[#C4A880]/10 to-purple-50 rounded-2xl p-6 ${className}`}>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          <Sparkles className="w-5 h-5 text-[#C4A880]" />
          <h3 className="font-medium text-gray-900">Jouw Stijl Ontdekking</h3>
        </div>
        <div className="flex items-center space-x-1">
          <Star className="w-4 h-4 text-yellow-400 fill-current" />
          <span className="text-sm font-medium text-gray-700">{overallConfidence}% Match</span>
        </div>
      </div>
        ...
        {insights.map((insight, index) => (
          <div key={index} className="flex items-center justify-between">
            <div>
              ...
              <span className="ml-2 font-medium text-gray-900">{insight.value}</span>
            </div>
            <div className="flex items-center space-x-2">
              <div className="w-16 bg-gray-200 rounded-full h-1.5">
                <div 
                  className="bg-[#C4A880] h-1.5 rounded-full transition-all duration-500"
