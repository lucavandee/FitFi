...
export function getCookiePrefs(): Prefs {
  return (
    readPrefs() ?? { analytics: false, marketing: false, consented: false }
  );
}
    ...
    <div className="fixed inset-x-0 bottom-0 z-[90] p-4">
      <div className="mx-auto max-w-3xl rounded-2xl border border-gray-200 bg-white p-4 shadow-xl">
        ...
        <p className="mt-1 text-sm text-gray-600">
          ...
          Beheer je voorkeuren hieronder. Zie ook onze{" "}
        ...
        <div className="mt-4 flex items-center gap-3">
          <button
            onClick={acceptAll}
            className="rounded-full bg-[#89CFF0] px-4 py-2 text-white btn-animate"
          >
            Alles accepteren
          </button>
          <button onClick={save} className="rounded-full border px-4 py-2">
            Opslaan
          </button>
        </div>
      </div>
