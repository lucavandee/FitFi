...
function TypingDots() {
  return (
    <span className="inline-flex gap-1 align-middle">
      <span className="animate-pulse">•</span><span className="animate-pulse" style={{animationDelay:".12s"}}>•</span><span className="animate-pulse" style={{animationDelay:".24s"}}>•</span>
    </span>
  );
}
...
function ChatPanel() {
  ...
  const bubble = (text: string, role: "user" | "assistant") => {
    const userCls = "inline-block rounded-2xl px-3 py-2 max-w-[85%] text-sm";
    if (role === "user") {
      return <div className="text-right"><div className={`${userCls}`} style={{background:"var(--ff-color-primary,#2B6AF3)", color:"#fff"}}>{text}</div></div>;
    }
    return <div className="text-left"><div className={`${userCls}`} style={{background:"#1b2138", color:"var(--ff-color-text,#E6E7EA)"}}>{text}</div></div>;
  };
  ...
  ), [minimized, toggleMinimize, setOpen]);
        ...
        <div className="relative w-full max-w-md bg-surface border rounded-xl shadow-lg overflow-hidden"
          ...
          <div ref={listRef} className="px-5 py-4 max-h-[55vh] overflow-y-auto space-y-3">
            {messages.length === 0 ? (
              ...
              messages.map((m) => <div key={m.id}>{bubble(m.text, m.role)}</div>)
            )}
