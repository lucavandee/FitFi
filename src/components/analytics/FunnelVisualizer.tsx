        ...
        {items.map((s, i) => {
          ...
          return (
            <li key={i} className="flex items-center gap-3">
              <div className="w-32 shrink-0 text-sm text-slate-700">{s.label}</div>
              <div className="flex-1 h-3 rounded-full bg-slate-100 overflow-hidden">
                <div className="h-full bg-[#9BD4F4]" style={{ width: `${pct}%` }} />
              </div>
              <div className="w-14 text-right text-sm text-slate-700 tabular-nums">{s.value}</div>
            </li>
          );
        })}
      </ol>
