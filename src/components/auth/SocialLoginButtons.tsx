...
export function SocialLoginButtons({ onSuccess, mode = 'register' }: SocialLoginButtonsProps) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      toast.error('Inloggen met Google mislukt. Probeer het opnieuw.');
      setLoading(false);
    }
  };
  ...
  const handleAppleLogin = async () => {
    try {
      ...
      toast.error('Inloggen met Apple mislukt. Probeer het opnieuw.');
      setLoading(false);
    }
  };

  const actionText = mode === 'login' ? 'Inloggen' : 'Ga verder';
  ...
  return (
    <div className="space-y-3">
      ...
      <motion.button
        ...
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-[var(--color-border)] rounded-lg bg-white hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
      >
