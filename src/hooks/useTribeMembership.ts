...
import {
  ...
  joinTribe,
  leaveTribe,
} from "@/services/tribes/tribeService";

export function useTribeMembership(tribeId: string, userId?: string) {
  const [members, setMembers] = useState(
    () => [] as Awaited<ReturnType<typeof getTribeMembers>>,
  );
  ...
  async function onJoin() {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[TribeMembership] Could not refresh after join, keeping optimistic state",
        );
  ...
  async function onLeave() {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "[TribeMembership] Could not refresh after leave, keeping optimistic state",
        );
        // Keep optimistic state if refresh fails
      }
    } catch (e) {
