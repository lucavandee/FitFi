...
export function useTribePosts(
  ...
  const [data, setData] = useState<TribePost[] | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [source, setSource] = useState<'supabase' | 'local' | 'fallback'>('fallback');
  const [cached, setCached] = useState(false);
  ...
  const loadPosts = async () => {
    ...
    try {
      ...
      const response = await fetchTribePosts(tribeId, fetchOptions);
  ...
  useEffect(() => {
    ...
    return () => cleanup.then(fn => fn?.());
  }, [tribeId, enabled, fetchOptions?.limit, fetchOptions?.offset, fetchOptions?.userId]);
