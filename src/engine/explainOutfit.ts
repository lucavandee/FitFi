...
export function generateOutfitExplanation(
  ...
  const archetypeExplanations: Record<string, string> = {
    klassiek:
      "Deze outfit past perfect bij jouw klassieke stijl door de tijdloze elegantie en verfijnde details.",
    casual_chic:
      "Deze combinatie belichaamt jouw casual chic aesthetic met moeiteloze elegantie en moderne touches.",
    urban:
      "Deze look sluit aan bij jouw urban stijl met functionele details en een stoere stadslook.",
    streetstyle:
      "Deze outfit past bij jouw streetstyle door de authentieke streetwear elementen en creatieve expressie.",
    retro:
      "Deze combinatie weerspiegelt jouw retro voorkeuren met vintage-geïnspireerde stukken en nostalgische details.",
    luxury:
      "Deze outfit past bij jouw luxury stijl door de exclusieve stukken en premium kwaliteit.",
  };
  ...
  const occasionExplanations: Record<string, string> = {
    ...
    Formeel:
      "De elegante details zorgen voor een geschikte look voor formele evenementen.",
    Casual:
      "De relaxte pasvorm en comfortabele materialen maken dit ideaal voor dagelijks gebruik.",
    Weekend: "De veelzijdige stukken zijn perfect voor een ontspannen weekend.",
    Uitgaan:
      "De statement pieces zorgen voor een opvallende look voor sociale gelegenheden.",
    Sport:
      "De functionele materialen en pasvorm ondersteunen een actieve lifestyle.",
  };
  ...
  if (outfit.products && outfit.products.length > 0) {
    ...
    if (uniqueCategories.length >= 3) {
      productInsights.push(
        "De complete outfit zorgt voor een gebalanceerde en doordachte look.",
      );
    }
    ...
    if (brands.length > 1) {
      productInsights.push(
        "De mix van verschillende merken toont jouw persoonlijke stijl.",
      );
    }
  }
  ...
  if (outfit.season) {
    const seasonMap: Record<string, string> = {
      spring:
        "De lichte materialen en frisse kleuren passen perfect bij het lenteweer.",
      summer:
        "De ademende stoffen en lichte tinten zijn ideaal voor warme zomerdagen.",
      autumn:
        "De warme lagen en rijke kleuren sluiten aan bij het herfstseizoen.",
      winter:
        "De isolerende materialen en donkere tinten bieden warmte en stijl in de winter.",
    };
...
export function generateNovaExplanation(
  ...
  if (userProfile?.stylePreferences) {
    const topPreference = Object.entries(userProfile.stylePreferences).sort(
      ([, a], [, b]) => b - a,
    )[0];
...
export function explainOutfit(o: any, opts: ExplainOpts = {}): OutfitExplain {
  ...
  if (o?.matchScore != null) {
    if (o.matchScore >= 80)
      rationale.push(
        'Sterke consistentie met je stijl-DNA; dit voelt meteen "juist".',
      );
    else if (o.matchScore >= 60)
      rationale.push(
        "Goede basis met ruimte voor nuance (accessoires of kleuraccent).",
      );
    else
      rationale.push(
        "Experimentele match — interessant voor variatie, maar minder kern-stijl.",
      );
  }

  // voorbeeld nuance
  rationale.push(
    "Tip: combineer één statement item met twee rustige basics voor balans.",
  );
