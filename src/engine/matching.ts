...
export function scoreAndFilterProducts(
  ...
  const mix = Object.keys(opts.archetypeMix ?? {}).length
    ? (opts.archetypeMix as ArchetypeWeights)
  ...
  scored.sort(
    (a, b) =>
      b.score - a.score || (a.product.price ?? 0) - (b.product.price ?? 0),
  );
...
export function buildOutfits(products: ProductLike[], opts: MatchOptions = {}) {
  ...
  const byCategory = ranked.reduce(
    (acc, { product }) => {
  ...
  for (
    let i = 0;
    i < Math.min(3, tops.length, bottoms.length, shoes.length);
    i++
  ) {
    const parts: ProductLike[] = [tops[i], bottoms[i], shoes[i]].filter(
      Boolean,
    );
    const details = parts.map((p) =>
      fusionScore(p, opts.archetypeMix ?? DEFAULT_ARCHETYPE_MIX),
    );
    const avg =
      details.reduce((a, d) => a + d.totalScore, 0) /
      Math.max(details.length, 1);
    const matchedSignals = Array.from(
      new Set(details.flatMap((d) => d.matchedSignals)),
    );
