...
import type { ArchetypeWeights, ProductLike, FusionScoreDetail } from '@/types/style';
...
export function normalizeWeights(weights: ArchetypeWeights): ArchetypeWeights {
  ...
  for (const [k, v] of entries) normalized[k as ArchetypeKey] = (v as number) / sum;
  return normalized;
}

function tokenMatchScore(tokens: string[] | undefined, needles: string[]): number {
...
export function scoreProductAgainstArchetype(p: ProductLike, key: ArchetypeKey): { score: number; signals: string[] } {
  ...
  const formScore = p.formality == null ? 0.5 : 1 - clamp01(Math.abs((p.formality - a.formality) / 100));
...
export function fusionScore(p: ProductLike, mix: ArchetypeWeights): FusionScoreDetail {
  ...
  return { totalScore: clamp01(total), byArchetype, matchedSignals: Array.from(new Set(signals)) };
}
...
export function formatBlendString(mix: ArchetypeWeights): string {
  ...
  const pairs = Object.entries(w)
    .sort((a, b) => b[1]! - a[1]!)
    .map(([k, v]) => `${Math.round((v as number) * 100)}% ${ARCHETYPES[k as ArchetypeKey].label}`);
  return pairs.join(' + ');
}