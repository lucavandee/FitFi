...
export function filterAndSortProducts(products: Product[], user: UserProfile): Product[] {
  ...
  if (!user || !user.stylePreferences) {
    console.warn('[FilterAndSort] No user or style preferences provided, returning all products');
    return products;
  }
  ...
  const validProducts = products.filter(product => {
    ...
    if (!product.id || !product.name) {
      console.warn('[FilterAndSort] Product missing required fields (id, name):', product);
      return false;
    }
  ...
  console.log(`[FilterAndSort] ${validProducts.length}/${products.length} products passed validation`);
  ...
  console.log(`[FilterAndSort] Final result: ${sortedProducts.length} products sorted by match score`);
...
export function groupProductsByCategory(products: Product[]): Record<ProductCategory, Product[]> {
  ...
  products.forEach(product => {
    ...
    try {
      ...
      console.warn('[GroupByCategory] Error categorizing product:', product.id, error);
...
export function getTopProductsByType(products: Product[], count: number = 2): Product[] {
  ...
  Object.entries(groupedProducts).forEach(([type, typeProducts]) => {
    ...
    console.log(`[TopByType] ${type}: selected ${topProducts.length}/${typeProducts.length} products`);
  });

  console.log(`[TopByType] Final result: ${result.length} products from ${Object.keys(groupedProducts).length} types`);
  return result;
}
...
export function getTopProductsByCategory(products: Product[], count: number = 2): Product[] {
  ...
  Object.entries(groupedProducts).forEach(([category, categoryProducts]) => {
    ...
    console.log(`[TopByCategory] ${category}: selected ${topProducts.length}/${categoryProducts.length} products`);
  });

  console.log(`[TopByCategory] Final result: ${result.length} products from ${Object.keys(groupedProducts).filter(k => groupedProducts[k as ProductCategory].length > 0).length} categories`);
  return result;
}
...
export function filterProductsByArchetype(
  ...
  console.log(`[ArchetypeFilter] ${archetype}: ${filteredProducts.length}/${products.length} products match (threshold: ${threshold})`);
...
export function validateProduct(product: any): { 
  ...
  if (!product.title && !product.name) errors.push('Missing required field: title or name');
  if (!product.category && !product.type) warnings.push('Missing category and type fields');
  ...
  if (product.gender && !['male', 'female', 'unisex'].includes(product.gender)) {
...
export function validateProductBatch(products: any[]): {
  ...
  products.forEach((product, index) => {
    ...
    if (validation.isValid) {
      ...
      invalidCount++;
      console.warn(`[BatchValidation] Product ${index} invalid:`, validation.errors);
    }

    if (validation.warnings.length > 0) {
      console.warn(`[BatchValidation] Product ${index} warnings:`, validation.warnings);
    }
  });

  console.log(`[BatchValidation] Summary: ${validCount} valid, ${invalidCount} invalid, ${totalErrors} errors, ${totalWarnings} warnings`);
