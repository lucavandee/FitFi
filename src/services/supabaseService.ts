...
export async function executeWithRetry<T>(
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      console.warn(
        `[SupabaseService] Retry ${attempt}/${maxAttempts} after ${delay}ms:`,
        error,
      );
    }
  }
...
export async function updateUserGamification(
  ...
  if (!sb) {
    console.warn(
      "[SupabaseService] Supabase not available for gamification update",
    );
    return null;
  }
...
export async function submitQuizAnswers(
  ...
  if (!sb) {
    console.warn(
      "[SupabaseService] Supabase not available for quiz submission",
    );
