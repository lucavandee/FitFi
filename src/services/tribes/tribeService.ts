...
function getClient() {
  ...
  if (!client) {
    console.log("[TribeService] Supabase not available - using localStorage fallback");
  }
  return client;
}
...
export async function getTribeMembers(tribeId: string): Promise<TribeMember[]> {
  ...
  if (sb && DATA_CONFIG.USE_SUPABASE) {
    try {
      ...
      console.log(`[TribeService] Loaded ${(data || []).length} members from Supabase`);
      return data || [];
    } catch (error) {
  ...
  console.log(`[TribeService] Using ${localMembers.length} members from localStorage`);
  return localMembers;
}
...
export async function getTribePosts(tribeId: string, options?: {
  ...
  if (sb && DATA_CONFIG.USE_SUPABASE) {
    try {
      ...
      console.log(`[TribeService] Loaded ${(data || []).length} posts from Supabase`);
      return data || [];
    } catch (error) {
  ...
  console.log(`[TribeService] Using ${localPosts.length} posts from localStorage`);
  return localPosts;
}
