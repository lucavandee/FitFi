...
export async function findBestOpenChallenge(
  userId?: string,
): Promise<{ tribeId?: string; challengeId?: string } | null> {
  const sb = supabase();
  let joined: string[] = [];
  if (sb && userId) {
    const { data } = await sb
  ...
  const fallbacks = joined.length
    ? []
    : ["tribe-italian-smart-casual", "tribe-streetstyle-europe"];
