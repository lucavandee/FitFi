...
function readLocalStats(userId: string) {
  return lsRead<UserStats | null>(LS_STATS(userId), {
    user_id: userId,
    level: 1,
    xp: 0,
...
function readLocalStreak(userId: string) {
  return lsRead<UserStreak>(LS_STREAK(userId), {
    user_id: userId,
    current_streak: 0,
    longest_streak: 0,
    last_check_date: new Date(0).toISOString(),
  });
}
  ...
  if (!sb) {
    ...
    if (last !== today) {
      ...
      current_streak =
        last && last !== today
          ? current_streak + 1
          : Math.max(1, current_streak + 1);
      longest_streak = Math.max(longest_streak, current_streak);
    }
    return writeLocalStreak({
      user_id: userId,
      current_streak,
      longest_streak,
