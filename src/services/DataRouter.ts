  ...
  async getProducts(query?: any): Promise<Product[]> {
    try {
      ...
      return filtered.map((p) => ({
        ...
        description:
          p.description || `${p.title || p.name} van ${p.brand || "Unknown"}`,
        price: p.price,
        brand: p.brand,
...
export async function getOutfits(query?: {
  ...
  if (shouldUseSupabase() && isHealthy()) {
    try {
      ...
      if (result && result.length > 0) {
        console.log(
          `[🧠 DataRouter] Returning ${result.length} outfits from Supabase`,
        );
        return result;
      }
    } catch (error) {
      console.warn(
        "[DataRouter] Supabase outfits failed, falling back:",
        error,
      );
    }
  }
  ...
  console.log(
    `[🧠 DataRouter] Returning ${result.length} outfits from local fallback`,
  );
  return result;
}
...
export async function getRecommendedProducts(
  ...
  console.log(
    "[🔍 DataRouter] getRecommendedProducts called with user:",
    user?.id || "anon",
    "count:",
    count,
    "season:",
  ...
  if (shouldUseSupabase() && isHealthy()) {
    try {
      ...
      if (result && result.length > 0) {
        console.log(
          `[🧠 DataRouter] Returning ${result.length} products from Supabase`,
        );
        return result;
      }
    } catch (error) {
      console.warn(
        "[DataRouter] Supabase products failed, falling back:",
        error,
      );
    }
  }
  ...
  try {
    ...
    console.log(
      `[🧠 DataRouter] Returning ${result.length} products from local fallback`,
    );
    return result;
  } catch (error) {
    ...
    const mockProducts = generateMockProducts(undefined, count);
    console.log(
      `[🧠 DataRouter] Returning ${mockProducts.length} mock products as final fallback`,
    );
    return mockProducts;
  }
}
...
export async function getArchetypes(): Promise<any[]> {
    ...
    try {
      ...
      if (sb) {
        ...
        if (!error && data && data.length > 0) {
          console.log(
            `[🧠 DataRouter] Returning ${data.length} archetypes from Supabase`,
          );
          return data;
        }
      }
      ...
      console.warn(
        "[DataRouter] Supabase archetypes failed, falling back:",
        error,
      );
    }
  }
  ...
  try {
    ...
    if (Array.isArray(archetypes)) {
      console.log(
        `[🧠 DataRouter] Returning ${archetypes.length} archetypes from local`,
      );
      return archetypes;
    }
  } catch (error) {
  ...
  const fallbackArchetypes = [
    { id: "klassiek", name: "Klassiek", description: "Tijdloze elegantie" },
    {
      id: "casual_chic",
      name: "Casual Chic",
      description: "Moeiteloos elegant",
    },
    { id: "urban", name: "Urban", description: "Stoere stadslook" },
    {
      id: "streetstyle",
      name: "Streetstyle",
      description: "Authentieke streetwear",
    },
  ...
  console.log(
    `[🧠 DataRouter] Returning ${fallbackArchetypes.length} hardcoded archetypes`,
  );
...
export async function getStyleProfile(userId: string): Promise<any> {
  ...
  if (shouldUseSupabase() && isHealthy()) {
    try {
      ...
      console.warn(
        "[DataRouter] Supabase style profile failed, falling back:",
        error,
      );
    }
  }
...
export async function getTribes(query?: {
  ...
  if (shouldUseSupabase() && isHealthy()) {
    try {
      ...
      if (result && result.length > 0) {
        console.log(
          `[🧠 DataRouter] Returning ${result.length} tribes from Supabase`,
        );
        return result;
      }
    } catch (error) {
  ...
  console.log(
    `[🧠 DataRouter] Returning ${result.length} tribes from local fallback`,
  );
  return result;
}
...
export async function getDashboardStats(userId: string): Promise<any> {
  ...
  if (shouldUseSupabase() && isHealthy()) {
    try {
      ...
      console.warn(
        "[DataRouter] Supabase dashboard stats failed, falling back:",
        error,
      );
    }
  }
