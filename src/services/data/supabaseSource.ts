...
export async function getSbProducts(filters?: {
  ...
  try {
    ...
    if (validProducts.length !== products.length) {
      console.warn(
        `[SupabaseSource] Filtered out ${products.length - validProducts.length} invalid products`,
      );
    }
...
export async function getSbOutfits(filters?: {
  ...
  try {
    ...
    if (validOutfits.length !== outfits.length) {
      console.warn(
        `[SupabaseSource] Filtered out ${outfits.length - validOutfits.length} invalid outfits`,
      );
    }
 ...
 */
export async function getSbUser(
  userId: string,
): Promise<FitFiUserProfile | null> {
  const sb = getClient();
  if (!sb) return null;
...
export async function getSbTribes(filters?: {
  ...
  try {
    ...
    if (validTribes.length !== tribes.length) {
      console.warn(
        `[SupabaseSource] Filtered out ${tribes.length - validTribes.length} invalid tribes`,
      );
    }
 ...
 */
export async function getSbTribeBySlug(
  slug: string,
  userId?: string,
): Promise<Tribe | null> {
  const sb = getClient();
  ...
  try {
    ...
    if (userId && isValidUUID(userId)) {
      try {
        ...
        console.warn(
          "[SupabaseSource] Could not check membership:",
          membershipError,
        );
 ...
 */
export async function sb_getTribeMembers(
  tribeId: string,
 ...
 */
export async function sb_joinTribe(
  tribeId: string,
  userId: string,
): Promise<TribeMember | null> {
  const sb = getClient();
 ...
 */
export async function sb_leaveTribe(
  tribeId: string,
  userId: string,
): Promise<void> {
  const sb = getClient();
  ...
  try {
    await executeSupabaseOperationSafe(
      async () => {
        return await sb
          .from("tribe_members")
          .delete()
 ...
 * Get tribe posts from Supabase
 */
export async function sb_getTribePosts(
  tribeId: string,
  options?: {
    limit?: number;
    offset?: number;
  ...
  try {
    ...
    console.log(
      `[SupabaseSource] Loaded ${(data || []).length} tribe posts for ${tribeId}`,
    );
 ...
 */
export async function sb_createTribePost(
  post: Omit<TribePost, "id" | "createdAt">,
): Promise<TribePost | null> {
  const sb = getClient();
  if (!sb) return null;
  ...
  if (!isValidUUID(post.user_id || "")) {
    console.warn(
      "[SupabaseSource] Invalid user ID format for post:",
      post.user_id,
    );
    return null;
  }
 ...
 */
export async function sb_deleteTribePost(
  postId: string,
  authorId: string,
): Promise<void> {
  const sb = getClient();
  ...
  if (!isValidUUID(authorId)) {
    console.warn(
      "[SupabaseSource] Invalid author ID format for delete:",
      authorId,
    );
    return;
  }
...
export async function getSbTribeChallenges(
  ...
  try {
    ...
    console.log(
      `[SupabaseSource] Loaded ${(data || []).length} challenges for tribe ${tribeId}`,
    );
...
export async function getSbChallengeSubmissions(
  ...
  try {
    ...
    console.log(
      `[SupabaseSource] Loaded ${(data || []).length} submissions for challenge ${challengeId}`,
    );
    return (data || []) as TribeChallengeSubmission[];
  } catch (error) {
    console.error(
      "[SupabaseSource] Challenge submissions fetch failed:",
      error,
    );
    throw error;
  }
...
export async function createSbChallengeSubmission(
  ...
  if (!isValidUUID(submission.userId)) {
    console.warn(
      "[SupabaseSource] Invalid user ID format for submission:",
      submission.userId,
    );
    return null;
  }
  ...
  try {
    ...
    console.error(
      "[SupabaseSource] Challenge submission creation failed:",
      error,
    );
    throw error;
  }
 ...
 */
export async function getSbTribeRankings(options?: {
  limit?: number;
  userId?: string;
  tribeId?: string;
...
export function clearSupabaseCache(): void {
  ...
  console.log(
    "[SupabaseSource] Cache cleared (no client-side cache for Supabase)",
  );
}
