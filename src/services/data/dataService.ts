...
const cache = new Map<
  string,
  { data: any; timestamp: number; source: "supabase" | "local" | "fallback" }
>();
...
function getCachedData<T>(
  key: string,
): { data: T; source: "supabase" | "local" | "fallback"; cached: true } | null {
...
export async function fetchProducts(options?: {
  ...
  try {
    const localData = await loadLocalJSON<BoltProduct[]>(
    ...
    errors.push(
      `Local JSON: ${error instanceof Error ? error.message : "Load failed"}`,
    );
  }
...
export async function fetchOutfits(options?: {
  ...
  // Fallback to local JSON
  try {
    const localData = await loadLocalJSON<Outfit[]>(
      DATA_CONFIG.LOCAL_JSON.outfits,
    );
...
export async function fetchUser(
  userId: string,
): Promise<DataResponse<FitFiUserProfile | null>> {
  ...
  // Fallback to local JSON
  try {
    const localData = await loadLocalJSON<FitFiUserProfile>(
      DATA_CONFIG.LOCAL_JSON.user,
    );
...
export async function fetchTribes(options?: {
  ...
  // Fallback to local JSON
  try {
    const localData = await loadLocalJSON<Tribe[]>(
      DATA_CONFIG.LOCAL_JSON.tribes,
    );
 ...
 */
export async function fetchTribeBySlug(
  slug: string,
  userId?: string,
): Promise<DataResponse<Tribe | null>> {
    ...
    try {
      ...
      if (sb) {
        ...
        if (!error && tribe) {
          ...
          const result = {
            data: tribeWithMembership as Tribe,
            source: "supabase" as const,
            cached: false,
          };
