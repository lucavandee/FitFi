...
// ---------- Challenges ----------
export async function getTribeChallenges(tribeId?: string): Promise<TribeChallenge[]> {
  const sb = supabase();
  try {
    if (sb) {
      let q = sb.from(T.tribeChallenges).select("*").order("startAt", { ascending: false });
    ...
    const all = await loadLocalJSON<TribeChallenge[]>(DATA_CONFIG.LOCAL_JSON.tribeChallenges);
...
export async function createTribeChallenge(ch: TribeChallenge): Promise<TribeChallenge> {
  const sb = supabase(); if (!sb) throw new Error("No Supabase client");
  const { data, error } = await sb.from(T.tribeChallenges).insert(ch).select("*").single();
  if (error) throw error;
  return data as TribeChallenge;
}
...
export async function updateTribeChallengeStatus(
  ...
  const sb = supabase(); if (!sb) throw new Error("No Supabase client");
  const { data, error } = await sb.from(T.tribeChallenges).update({ status }).eq("id", id).select().single();
  if (error) throw error;
  return data as TribeChallenge;
}

// ---------- Submissions ----------
export async function getChallengeSubmissions(challengeId: string): Promise<TribeChallengeSubmission[]> {
  const sb = supabase();
  try {
    if (sb) {
      const { data, error } = await sb.from(T.tribeChallengeSubmissions).select("*").eq("challengeId", challengeId).order("createdAt", { ascending: false });
      if (error) throw error;
      return (data ?? []) as TribeChallengeSubmission[];
    }
    throw new Error("No Supabase client");
  } catch {
    const all = await loadLocalJSON<TribeChallengeSubmission[]>(DATA_CONFIG.LOCAL_JSON.tribeChallengeSubmissions);
    return all.filter(s => s.challengeId === challengeId);
  }
}

export async function createChallengeSubmission(sub: TribeChallengeSubmission): Promise<TribeChallengeSubmission> {
  const sb = supabase(); if (!sb) throw new Error("No Supabase client");
  const { data, error } = await sb.from(T.tribeChallengeSubmissions).insert(sub).select("*").single();
  if (error) throw error;
  return data as TribeChallengeSubmission;
}
...
export async function updateSubmissionScore(
  ...
  const sb = supabase(); if (!sb) throw new Error("No Supabase client");
  const { data, error } = await sb.from(T.tribeChallengeSubmissions).update({ score, isWinner }).eq("id", submissionId).select().single();
  if (error) throw error;
  return data as TribeChallengeSubmission;
}
...
export async function getTribeRanking(): Promise<TribeRanking[]> {
  ...
  try {
    if (sb) {
      const { data, error } = await sb.from(T.tribeRanking).select("*").order("points", { ascending: false });
      if (error) throw error;
