...
function brandHue(i: number){ const hues = [198, 210, 222, 230, 192, 204]; return hues[i % hues.length]; }
export function fallbackDataUrl(id: string, kind: ImageKind, w = 480, h = 640): string {
  const seed = hashString(`${kind}:${id}`);
  const c1 = `hsl(${brandHue(seed)} 70% 50%)`;
  const c2 = `hsl(${brandHue(seed >> 1)} 70% 42%)`;
  const label = kind === 'nova' ? 'Nova' : (kind[0].toUpperCase() + kind.slice(1));
