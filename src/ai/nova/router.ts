...
export function routeMessage(text: string, ctx: NovaContext): NovaReply {
  ...
  if (!ctx.userId) {
    try {
      localStorage.setItem("nova_pending_query", q);
    } catch {}
    return {
      type: "gate",
      text: "Log in of maak gratis een account. Dan stel ik direct 3 looks voor je samen op basis van je stijl en seizoen.",
    };
  }
