...
export const GamificationProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const levels: LevelInfo[] = [
    ...
    },
    {
      ...
      icon: "🔍",
      color: "#3b82f6",
      perks: ["Daily challenges", "Basic analytics"],
    },
    {
      id: 3,
      ...
      icon: "✨",
      color: "#8b5cf6",
      perks: ["Weekly challenges", "Outfit history"],
    },
    {
      id: 4,
      level_name: "Trendsetter",
      min_xp: 600,
      max_xp: 1000,
      icon: "🎯",
      color: "#f59e0b",
      perks: ["Premium challenges", "Style insights"],
    },
    {
      id: 5,
      level_name: "Influencer",
      min_xp: 1000,
      icon: "⭐",
      color: "#ef4444",
  ...
  const currentLevelInfo =
    levels.find((l) => l.level_name.toLowerCase() === data.level) || levels[0];
  const nextLevelInfo =
    levels.find((l) => l.id === (currentLevelInfo?.id || 0) + 1) || null;

  const progressToNextLevel = nextLevelInfo
    ? Math.round(
        ((data.points - currentLevelInfo.min_xp) /
          (nextLevelInfo.min_xp - currentLevelInfo.min_xp)) *
          100,
      )
    : 100;
