...
import {
  ...
  useCreateChallengeSubmission,
  useCreateTribeChallenge,
} from "../hooks/useTribeChallenges";
...
const ChallengeSection: React.FC<{
  ...
  const { data: subs, loading: subsLoading } = useChallengeSubmissions(
    challenge.id,
  );
  ...
  async function onSubmit(payload: {
    ...
    try {
      await createSub.mutateAsync({
        ...
        isWinner: false,
        submissionType:
          payload.imageUrl && payload.linkUrl
            ? "combo"
            : payload.imageUrl
              ? "image"
              : payload.linkUrl
...
const AdminChallengeControls: React.FC<{
  challenge: TribeChallenge;
  onUpdate: () => void;
}> = ({ challenge, onUpdate }) => {
      ...
      <div className="min-h-screen bg-[#F6F6F6] flex items-center justify-center">
        <div className="bg-white p-8 rounded-3xl shadow-sm text-center max-w-md">
          ...
          <p className="text-gray-600 mb-6">
            Deze tribe bestaat niet of is niet beschikbaar.
          </p>
  ...
  return (
    <div className="min-h-screen bg-[#F6F6F6]">
      <Helmet>
        ...
        <link
          rel="canonical"
          href={`https://fitfi.app/tribes/${slug || tribeId}`}
        />
      </Helmet>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600">
                    Verdien punten en toon je stijl
                  </p>
                </div>
              </div>
              ...
              {challenges && challenges.length > 0 && (
                <div className="text-sm text-gray-600">
                  {challenges.filter((c) => c.status === "open").length} actieve
                  challenges
                </div>
              )}
            </div>
        ...
        <PostsList tribeId={tribe.id} userId={user?.id} showComposer={false} />
      </div>
    </div>
  );
};
