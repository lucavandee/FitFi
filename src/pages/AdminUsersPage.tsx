...
function StatCard({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    ...
    >
      <div className="text-xs text-[#6E6E6E] uppercase tracking-wider mb-1">{label}</div>
...
function SortButton({ field, current, dir, onClick }: {
  ...
  return (
    <button onClick={() => onClick(field)} className="inline-flex items-center gap-0.5 hover:text-[#9A503B] transition-colors">
      {active ? (
    ...
    return (
      <div className="flex items-center justify-center p-6" style={{ minHeight: 'calc(100vh - 72px)' }}>
        <div className="text-center">
          <XCircle className="w-16 h-16 text-[#6E6E6E] mx-auto mb-4" />
          <h1 className="text-xl font-bold text-[#1A1A1A] mb-2">Geen toegang</h1>
          <p className="text-[#6E6E6E]">Je hebt geen admin rechten.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-[#FFFFFF] border border-[#E5E5E5] rounded-xl p-4 mb-4 shadow-sm">
          ...
          {(searchTerm || filterTier !== 'all' || filterQuiz !== 'all') && (
            <div className="mt-2 flex items-center gap-2 text-xs text-[#6E6E6E]">
        ...
        <div className="bg-[#FFFFFF] border border-[#E5E5E5] rounded-xl overflow-hidden shadow-sm">
          {loading ? (
            <div className="flex items-center justify-center py-16">
              <div className="animate-spin w-8 h-8 border-4 border-[#9A503B] border-t-transparent rounded-full" />
            </div>
          ) : (
            <>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-[#FAF5F2] border-b border-[#E5E5E5]">
                      ...
                      <th className="px-5 py-3 text-left text-xs font-semibold text-[#6E6E6E] uppercase tracking-wider hidden md:table-cell">
                        Abonnement
                      </th>
                      <th className="px-5 py-3 text-left text-xs font-semibold text-[#6E6E6E] uppercase tracking-wider hidden lg:table-cell">
                        Quiz
                      </th>
                      <th className="px-5 py-3 text-left text-xs font-semibold text-[#6E6E6E] uppercase tracking-wider hidden lg:table-cell">
                        <div className="flex items-center gap-1">
                      ...
                      <th className="px-5 py-3 text-left text-xs font-semibold text-[#6E6E6E] uppercase tracking-wider hidden xl:table-cell">
                        <div className="flex items-center gap-1">
                          Laatste login
                          <SortButton field="last_sign_in" current={sortField} dir={sortDir} onClick={handleSort} />
                        </div>
                      </th>
                      <th className="px-5 py-3 text-right text-xs font-semibold text-[#6E6E6E] uppercase tracking-wider">
                        Acties
                      </th>
                    </tr>
                  </thead>
                    ...
                    {paginated.map((u) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-5 py-3.5 text-xs text-[#6E6E6E] hidden lg:table-cell">
                            {new Date(u.created_at).toLocaleDateString('nl-NL')}
                          </td>
                          <td className="px-5 py-3.5 text-xs text-[#6E6E6E] hidden xl:table-cell">
                              ...
                              {openMenuId === u.id && (
                                <>
                                  ...
                                  <div className="absolute right-0 top-8 z-20 bg-[#FFFFFF] border border-[#E5E5E5] rounded-xl shadow-lg py-1 w-48">
                                    <div className="px-3 py-1.5 text-xs font-semibold text-[#6E6E6E] uppercase tracking-wider">Tier wijzigen</div>
                                    {(['free', 'premium', 'founder'] as const).map((t) => (
                                      ...
                                      >
                                        ...
                                        {QUICK_TIER_LABELS[t]}
                                        {u.tier === t && <span className="ml-auto text-xs text-[#6E6E6E]">huidig</span>}
                                      </button>
                                    ))}
              ...
              {paginated.length === 0 && (
                <div className="text-center py-16">
                  <Users className="w-12 h-12 text-[#6E6E6E] mx-auto mb-3 opacity-40" />
                  <p className="text-[#6E6E6E]">Geen gebruikers gevonden</p>
                </div>
              )}
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-5 py-3 border-t border-[#E5E5E5] bg-[#FAF5F2]">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-[#6E6E6E]">{page + 1} / {totalPages}</span>
                    <button
