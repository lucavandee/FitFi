...
const TIER_BADGE: Record<string, { label: string; cls: string }> = {
  ...
  free:    { label: 'Free',    cls: 'bg-[#FAF5F2] text-[#8A8A8A] border border-[#E5E5E5]' },
};
...
function StatCard({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    <motion.div
      ...
      className="bg-[#FFFFFF] border border-[#E5E5E5] rounded-xl p-5 shadow-sm"
    >
      <div className="text-xs text-[#8A8A8A] uppercase tracking-wider mb-1">{label}</div>
      <div className={`text-3xl font-bold ${color || 'text-[#1A1A1A]'}`}>{value}</div>
    </motion.div>
  );
}
...
function SortButton({ field, current, dir, onClick }: {
  ...
  return (
    <button onClick={() => onClick(field)} className="inline-flex items-center gap-0.5 hover:text-[#A8513A] transition-colors">
      {active ? (
    ...
    return (
      <div className="flex items-center justify-center p-6" style={{ minHeight: 'calc(100vh - 72px)' }}>
        <div className="text-center">
          <XCircle className="w-16 h-16 text-[#8A8A8A] mx-auto mb-4" />
          <h1 className="text-xl font-bold text-[#1A1A1A] mb-2">Geen toegang</h1>
          <p className="text-[#8A8A8A]">Je hebt geen admin rechten.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-[#FAFAF8]" style={{ minHeight: 'calc(100vh - 72px)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <Users className="w-6 h-6 text-[#C2654A]" />
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 rounded-lg border border-[#E5E5E5] hover:bg-[#FAF5F2] transition-colors"
            >
              <RefreshCw className={`w-4 h-4 text-[#8A8A8A] ${loading ? 'animate-spin' : ''}`} />
            </button>
            <button
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 mb-8">
          ...
          <StatCard label="Premium" value={stats.premium} color="text-[#6366F1]" />
        ...
        {/* Filters */}
        <div className="bg-[#FFFFFF] border border-[#E5E5E5] rounded-xl p-4 mb-4 shadow-sm">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 text-sm border border-[#E5E5E5] rounded-lg focus:ring-2 focus:ring-[#C2654A] focus:border-transparent bg-[#FFFFFF]"
              />
            </div>
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="px-3 py-2.5 text-sm border border-[#E5E5E5] rounded-lg focus:ring-2 focus:ring-[#C2654A] focus:border-transparent bg-[#FFFFFF]"
              >
        ...
        {/* Table */}
        <div className="bg-[#FFFFFF] border border-[#E5E5E5] rounded-xl overflow-hidden shadow-sm">
          {loading ? (
            ...
            <>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-[#FAF5F2] border-b border-[#E5E5E5]">
                      <th className="px-5 py-3 text-left text-xs font-semibold text-[#8A8A8A] uppercase tracking-wider">
                      ...
                      <th className="px-5 py-3 text-left text-xs font-semibold text-[#8A8A8A] uppercase tracking-wider hidden md:table-cell">
                        Abonnement
                      </th>
                      <th className="px-5 py-3 text-left text-xs font-semibold text-[#8A8A8A] uppercase tracking-wider hidden lg:table-cell">
                        Quiz
                      </th>
                      <th className="px-5 py-3 text-left text-xs font-semibold text-[#8A8A8A] uppercase tracking-wider hidden lg:table-cell">
                        <div className="flex items-center gap-1">
                      ...
                      <th className="px-5 py-3 text-left text-xs font-semibold text-[#8A8A8A] uppercase tracking-wider hidden xl:table-cell">
                        <div className="flex items-center gap-1">
                          Laatste login
                          <SortButton field="last_sign_in" current={sortField} dir={sortDir} onClick={handleSort} />
                        </div>
                      </th>
                      <th className="px-5 py-3 text-right text-xs font-semibold text-[#8A8A8A] uppercase tracking-wider">
                        Acties
                      </th>
                    </tr>
                  </thead>
                        ...
                        >
                          <td className="px-5 py-3.5">
                            <div className="flex items-center gap-2.5">
                              <div className="w-8 h-8 rounded-full bg-[#FAF5F2] flex items-center justify-center flex-shrink-0 text-xs font-bold text-[#A8513A]">
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="text-xs text-[#8A8A8A] truncate">{u.email}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-5 py-3.5">
                          ...
                          <td className="px-5 py-3.5 text-xs text-[#8A8A8A] hidden lg:table-cell">
                            {new Date(u.created_at).toLocaleDateString('nl-NL')}
                          </td>
                          <td className="px-5 py-3.5 text-xs text-[#8A8A8A] hidden xl:table-cell">
                            ...
                            <div className="relative inline-block">
                              ...
                              {openMenuId === u.id && (
                                <>
                                  ...
                                  <div className="absolute right-0 top-8 z-20 bg-[#FFFFFF] border border-[#E5E5E5] rounded-xl shadow-lg py-1 w-48">
                                    <div className="px-3 py-1.5 text-xs font-semibold text-[#8A8A8A] uppercase tracking-wider">Tier wijzigen</div>
                                    {(['free', 'premium', 'founder'] as const).map((t) => (
                                      ...
                                      >
                                        ...
                                        {QUICK_TIER_LABELS[t]}
                                        {u.tier === t && <span className="ml-auto text-xs text-[#8A8A8A]">huidig</span>}
                                      </button>
                                    ))}
              ...
              {paginated.length === 0 && (
                <div className="text-center py-16">
                  <Users className="w-12 h-12 text-[#8A8A8A] mx-auto mb-3 opacity-40" />
                  <p className="text-[#8A8A8A]">Geen gebruikers gevonden</p>
                </div>
              )}
                ...
                <div className="flex items-center justify-between px-5 py-3 border-t border-[#E5E5E5] bg-[#FAF5F2]">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      disabled={page === 0}
                      className="px-3 py-1.5 text-xs rounded-lg border border-[#E5E5E5] disabled:opacity-40 hover:bg-[#FFFFFF] transition-colors"
                    >
                      Vorige
                    </button>
                    <span className="text-xs text-[#8A8A8A]">{page + 1} / {totalPages}</span>
                    <button
