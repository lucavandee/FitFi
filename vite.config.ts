...
// (blijft staan) FitFi Guard — blokkeer letterlijke "..." in /src/
function fitfiGuard() {
  return {
    name: "fitfi-guard",
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    // ⛳️ CRUCIALE REGEL: forceer inline TypeScript-config
    // Hierdoor wordt /tsconfig.json NIET meer ingelezen en kan "extends" je build niet meer breken.
    esbuild: {
      tsconfigRaw: {
        compilerOptions: {
          target: "ES2020",
